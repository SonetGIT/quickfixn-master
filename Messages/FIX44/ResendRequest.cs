...
namespace QuickFix
{
    namespace FIX44 
    {
        ...
        {
            /*
             Сообщение используется для инициирования повторной пересылки сообщений в определённом интервале номеров. Если нужно
             запросить повторную пересылку одного сообщения, тогда BeginSeqNo=EndSeqNo. Если нужно запросить все последующие сооб-
             щения, начиная с определённого номера, тогда EndSeqNo=0 (показывает бесконечность).
            */
