...
export default (props) => {  
  ...
  async function fetchForm(defid){
    await fetch(
      kfbRESTApi + "/api/Metadata/GetByDefId?defid=" + defid,
      {
  ...
  //Фун-я обновления данных
  async function updateDocREST(api, body){
    await fetch(
      kfbRESTApi + api,
      {
        "headers": { "content-type": "application/json" },
  ...
  //Фун-я сохранения данных
  async function saveDocREST(api, body){
    await fetch(kfbRESTApi + api,
      {
        "headers": { "content-type": "application/json" },
        "method": "POST",
        "body": body
  ...
  //Фун-я обновления данных
  async function deleteDocREST(api, body){
    await fetch(
      kfbRESTApi + api,
      {
        "headers": { "content-type": "application/json" },
  ...
  function buttonClick(buttonName, item){
    ...
    //Назад к главному справочнику
      ...
      props.setNewTask("backToMainRef", taskVars)
    ...
    //Открыть форму редактирования элемента справочника
    ...
    //Назад к списку выбранного справочника
      ...
      props.setNewTask("backToRefSearchForm", taskVars)
      // console.log("button backToRefSearchForm: ", taskVars)   
    }       
    //Открыть форму создания спр
    else if(buttonName === "showCreateRef"){     
      let taskVars = {
      ...
      props.setNewTask("showCreateRef", taskVars)
      console.log("button showCreateRef: ", taskVars)
    }
    //Вносить изменения данных и сохранить спр
    else if(buttonName === "updateRef"){
      let updateBody = getFieldValuesUpdateDoc()
      updateDocREST(taskVariables.controller + "?id=" + selectedDoc.id, JSON.stringify(updateBody))      
      console.log("button updateRef: ")   
    }
    //Сохранения спр
    else if(buttonName === "saveRef"){
      let saveBody = getFieldValuesSaveDoc()      
      saveDocREST(taskVariables.controller, JSON.stringify(saveBody))
    ...
    //Удалить данные из спр
      ...
      deleteDocREST(taskVariables.controller + "?id=" + selectedDoc.id, JSON.stringify(docToDelete))      
      console.log("button deleteRef: ")   
    }
    //Форма редактирования пользователя
      ...
      props.setNewTask("showEditUserForm", taskVars)
    ...
    //Назад к списку пользователей
      ...
      props.setNewTask("backToUserSearchForm", taskVars)
      console.log("button backToUserSearchForm: ", taskVars)
    }
    //Вносить изменения данных и сохранить спр
    ...
    //Сохранения спр
      ...
      if(checkResult === true){   
      saveDocREST("/api/users/Registration", JSON.stringify(saveBody))
      console.log("button saveUser: Данные сохранены" )
      }
    }
